...
router.get('/admin/leave-requests', async (req, res) => {
  try {
    const leaveRequests = await CustomizeLeave.find({ approvalStatus: 'Pending' }); // Only pending leave requests
...
router.post('/admin/update-leave/:id', async (req, res) => {
  try {
    const { fromDate, toDate, numOfDays, reason, designation, reportingHeadSignature, reportingHeadReason, sanctioningAuthoritySignature, sanctioningAuthorityReason } = req.body;
...
router.post('/admin/approve-reject/:id', async (req, res) => {
  try {
    const leaveRequest = await CustomizeLeave.findById(req.params.id);
    if (!leaveRequest) {
      return res.status(404).json({ message: 'Leave request not found' });
    ...
    const { status, reportingHeadSignature, reportingHeadReason, sanctioningAuthoritySignature, sanctioningAuthorityReason } = req.body;
    ...
    res.status(200).json({ message: 'Leave request updated and employee notified' });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Server error' });
  }
...
router.get('/employee/:employeeId/:leaveType', async (req, res) => {
  try {
    ...
    if (employee) {
      ...
      } else if (leaveType === 'PL') {  // Changed from SL to PL
    ...
    return res.status(500).json({ exists: false, message: 'Error fetching employee details' });
  }
});

router.post('/submit-leave', upload.array('attachments'), async (req, res) => {
  try {
    const { employeeId, name, leaveType, fromDate, toDate, numOfDays, reason, designation } = req.body;
    
    const fromDateParsed = new Date(fromDate);
    const toDateParsed = new Date(toDate);

    const attachments = Array.isArray(req.files) ? req.files.map(file => file.path) : [];

    if (!employeeId || !name || !leaveType || !fromDate || !toDate || !numOfDays || !reason || !designation) {
      return res.status(400).json({ success: false, message: 'Missing required fields' });
    }

    if (leaveType === 'ML' && numOfDays > 2 && attachments.length === 0) {
      return res.status(400).json({ success: false, message: 'Attachment is required for ML leave when number of days is greater than 2' });
    }

    if (leaveType === 'PL' && attachments.length === 0) {  // Changed from SL to PL
      return res.status(400).json({ success: false, message: 'Attachment is required for PL leave' });  // Changed from SL to PL
    }

    if (leaveType === 'CL' && attachments.length > 0) {
      return res.status(400).json({ success: false, message: 'Attachment should not be added for CL leave' });
    }
